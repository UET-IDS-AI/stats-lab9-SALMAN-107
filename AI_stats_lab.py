...
def correlation_xy():
    ...
    if var_x == 0 or var_y == 0:
        return 0.0
    return covariance_xy() / np.sqrt(var_x * var_y)
...
def variance_sum():
    ...
    for x in range(4):
        for y in range(4):
            expected_sum += ((x + y) ** 2) * joint_pmf(x, y)
    mean_sum = expected_x() + expected_y()
    return expected_sum - (mean_sum ** 2)
...
def variance_identity_check():
    ...
    """
    var_sum = variance_sum()
    identity = variance_x() + variance_y() + 2 * covariance_xy()
    return bool(np.isclose(var_sum, identity))
